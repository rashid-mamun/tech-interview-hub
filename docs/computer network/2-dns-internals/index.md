...
| :--- | :--- | :--- |
| **সংজ্ঞা** | এটি হলো DNS ডেটাবেসের একটি সিঙ্গেল এন্ট্রি বা তথ্য (যেমন A রেকর্ড, MX রেকর্ড)। | এটি হলো একটি ডোমেইন স্পেসের অংশ, যার পরিচালনার দায়িত্ব নির্দিষ্ট কোনো অ্যাডমিনিস্ট্রেটরের কাছে থাকে। |
| **কাজ** | এটি নির্দিষ্ট একটি ডোমেইনের কোনো একটি প্রপার্টি ডিফাইন করে। | একটি DNS Zone-এর ভেতরে ওই ডোমেইন এবং তার সাবডোমেইনের সমস্ত DNS Record গুলোকে একটি ফাইলে (Zone File) সংগঠিত রাখা হয়। |
...
* **▶️ Forward DNS Lookup:** ডোমেইন নেম (যেমন `google.com`) থেকে IP অ্যাড্রেস বের করার প্রক্রিয়াকে ফরোয়ার্ড লুকআপ বলে। এটিই সবচেয়ে বেশি ব্যবহৃত হয়।
* **◀️ Reverse DNS Lookup (rDNS):** IP অ্যাড্রেস (যেমন `142.250.190.46`) থেকে সেই আইপিটি কোন ডোমেইনের সাথে যুক্ত, তা বের করার প্রক্রিয়াকে রিভার্স লুকআপ বলে। এটি স্প্যাম চেকিং এবং ট্রাবলশুটিং-এর কাজে ব্যবহৃত হয়।
...
| :--- | :--- | :--- |
| **সংজ্ঞা** | এটি হলো ডেটাবেসের সেই অরিজিনাল সার্ভার, যার কাছে নির্দিষ্ট ডোমেইনটির জন্য সর্বশেষ এবং সঠিক DNS রেকর্ডগুলো জমা থাকে। এটিই চূড়ান্ত উত্তর দেয়। | এটি হলো ইউজারের ডিভাইস (বা ISP) থেকে পাঠানো রিকোয়েস্ট গ্রহণকারী সাভার। |
| **কাজের ধরন** | এর কাছে সব রেকর্ড সেভ করা থাকে। | এর নিজের কাছে সব উত্তর থাকে না, বরং ইউজারের হয়ে এটি অন্যান্য সার্ভারকে পর্যায়ক্রমে প্রশ্ন করে আইপি অ্যাড্রেসটি খুঁজে এনে ইউজারকে দেয়। |
...
3. **🏷️ Root to TLD Server:** রুট সার্ভার আইপি জানে না, কিন্তু সে `.com`-এর দায়িত্বে থাকা **TLD (Top-Level Domain) Server**-এর ঠিকানা বলে দেবে।
...
* **CNAME-এর সীমাবদ্ধতা:** DNS রুলস অনুযায়ী, রুট বা এপেক্স ডোমেইনে (যেমন শুধু `example.com`) কোনো CNAME রেকর্ড অ্যাড করা যায় শারীরিক বা লজিক্যাল বসানো যায় না, CNAME শুধু সাবডোমেইনে (যেমন `www.example.com`) ব্যবহার করা যায়। 
...
DNS একটি **Tree-shaped** অনুসরণ করে। একদম উপরে Root, তারপর TLD, তারপর Authoritative — এভাবে ধাপে ধাপে নিচে নামে।

#### Root DNS Server

DNS হায়ারার্কির একদম **শীর্ষে** অবস্থান। এটিকে `.` (dot) দিয়ে প্রকাশ করা হয়। বিশ্বে মাত্র **১৩টি Root Server cluster** আছে (A থেকে M পর্যন্ত), যদিও আসল সার্ভারের সংখ্যা ৬০০+।
...
তাছাড়া Resolver-এ **Cache** থাকে — তাই Root Server ছাড়াও কিছুক্ষণ ইন্টারনেট চলতে পারে।

> 💡 **সহজ কথা:** Root Server ১৩টি *নামে*, কিন্তু ৬০০+ *শরীরে* — বিশ্বজুড়ে ছড়িয়ে আছে। ICANN তত্ত্বাবধান করে, কিন্তু একা নিয়ন্ত্রণ করে না — এটাই ইন্টারনেটের শক্তি।
...
* **🏴‍☠️ DNS Spoofing (বা Cache Poisoning):** এই আক্রমণে হ্যাকার কোনোভাবে একটি রিসলভার এর ক্যাশে ভুয়া বা ফেক আইপি (Fake IP) ঢুকিয়ে দেয়। এর ফলে ইউজার যখন বৈধ ডোমেইনে (যেমন `bank.com`) ভিজিট করতে চায়, রিসলভার ক্যাশ থেকে হ্যাকারের বসানো ভুয়া আইপি দেয় এবং ইউজার ওই ফেক ওয়েবসাইটে গিয়ে পাসওয়ার্ড দিয়ে দিলে হ্যাকার তা পেয়ে যায়।
